...
export default function AdminDashboard({ onLogout }) {
  ...
  const approveRequest = async (id) => {
    try {
      const res = await fetch(`${backendBaseUrl}/api/requests/${id}/approve`, {
        method: "PUT",
  ...
  const confirmDelete = async () => {
    if (!deleteId) return;
    try {
      const res = await fetch(`${backendBaseUrl}/api/requests/${deleteId}`, {
        method: "DELETE",
      });
      const data = await res.json();
      if (res.ok && data.success) {
    ...
    <div className="min-h-screen flex flex-col md:flex-row bg-gradient-to-br from-black via-gray-900 to-green-900 relative">
      ...
      >
        <div className="flex flex-col h-full">
          ...
          <div className="px-4 py-4 border-t border-green-500/40 mt-auto">
            <button
              onClick={onLogout}
      ...
      <div className="flex-1 mt-12 md:mt-0 h-[100vh] overflow-auto relative">
        ...
        {activePage === "create" && <CreateCertificatePage formData={formData} setFormData={setFormData} />}
      ...
      {showConfirm && (
        <div className="fixed inset-0 bg-black/50 backdrop-blur-sm flex items-center justify-center z-50">
          <div className="bg-gray-900 p-6 rounded-lg shadow-lg max-w-sm w-full">
            ...
            <p className="text-gray-300 mb-6">Are you sure you want to delete this request?</p>
      ...
      {showApproveConfirm && (
        <div className="fixed inset-0 bg-black/50 backdrop-blur-sm flex items-center justify-center z-50">
          <div className="bg-gray-900 p-6 rounded-lg shadow-lg max-w-sm w-full">
            <h2 className="text-lg font-semibold text-white mb-4">Confirm Approve</h2>
            <p className="text-gray-300 mb-6">Are you sure you want to approve this request?</p>
            <div className="flex justify-end gap-3">
              <button
