...
export default function UserRequests({ copyRequestToForm }) {
  ...
  useEffect(() => {
    fetch("http://localhost:5000/api/certificates/requests")
      .then((res) => {
  ...
  const handleDelete = (id) => {
    ...
    // ✅ Optional: also send DELETE request to backend
    // fetch(`http://localhost:5000/api/certificates/requests/${id}`, { method: "DELETE" })
  };
            ...
            >
              <div className="flex justify-between items-center">
                ...
                <div className="flex gap-3">
                  ...
                  <button
                    onClick={() =>
                      setExpanded(expanded === (req.id || req._id) ? null : (req.id || req._id))
                    }
                    className="p-2 bg-indigo-500 text-white rounded-lg hover:bg-indigo-600 transition flex items-center justify-center"
                    title={expanded === (req.id || req._id) ? "Hide Details" : "View More"}
                  >
                    <EyeIcon className="h-5 w-5" />
                  </button>
